...
Ouvre les paramètres (⚙) après activation pour configurer le dossier et les règles.`,defaultEnabled:!1,hasSettings:!0}];L.map(e=>e.id);function R(){return Object.fromEntries(L.map(e=>[e.id,e.defaultEnabled]))}function z(e){let t=R();if(!e||typeof e!=`object`||Array.isArray(e))return t;let n={...t};for(let t of L)typeof e[t.id]==`boolean`&&(n[t.id]=e[t.id]);return n}function Oe(e,t){return typeof e?.[t]==`boolean`?e[t]:L.find(e=>e.id===t)?.defaultEnabled!==!1}var ke=new Set(_.map(e=>e.type));function B(e){let t=v();if(!e||typeof e!=`object`||Array.isArray(e))return t;let n=e;return{folderName:typeof n.folderName==`string`&&n.folderName.trim()?n.folderName.trim():t.folderName,folderId:typeof n.folderId==`string`?n.folderId:null,folderSource:n.folderSource===`existing`||n.folderSource===`create`?n.folderSource:t.folderSource,rules:Ae(n.rules)}}function Ae(e){if(!Array.isArray(e))return[];let t=[];for(let n of e){if(!n||typeof n!=`object`)continue;let e=n,r=e.type;if(typeof r!=`string`||!ke.has(r))continue;let i=String(e.templateNoteId??``).trim();if(!i)continue;let a={id:typeof e.id==`string`&&e.id?e.id:crypto.randomUUID(),type:r,templateNoteId:i,folderId:null,pattern:``};if(r===`folder`){let t=e.folderId??e.folder_id;if(typeof t!=`string`||!t)continue;a.folderId=t}else if(r===`title-exact`||r===`title-contains`){let t=String(e.pattern??``).trim();if(!t)continue;a.pattern=t}t.push(a)}return t}function V(e,t){let n=e.find(e=>e.id===t);return n?String(n.content_md??n.contentMd??``):``}function je(e=new Date){return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-${String(e.getDate()).padStart(2,`0`)}`}function Me(e={}){let t=String(e.title??``).trim(),n=e.date instanceof Date?e.date:new Date,r={titre:t,title:t,date:n.toLocaleDateString(`fr-FR`,{weekday:`long`,day:`numeric`,month:`long`,year:`numeric`}),"date-courte":n.toLocaleDateString(`fr-FR`),date_courte:n.toLocaleDateString(`fr-FR`),"date-short":n.toLocaleDateString(`fr-FR`),"date-iso":je(n),date_iso:je(n),jour:String(n.getDate()).padStart(2,`0`),day:String(n.getDate()).padStart(2,`0`),mois:String(n.getMonth()+1).padStart(2,`0`),month:String(n.getMonth()+1).padStart(2,`0`),annee:String(n.getFullYear()),year:String(n.getFullYear()),heure:n.toLocaleTimeString(`fr-FR`,{hour:`2-digit`,minute:`2-digit`}),time:n.toLocaleTimeString(`fr-FR`,{hour:`2-digit`,minute:`2-digit`}),"heure-complete":n.toLocaleTimeString(`fr-FR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}),heure_complete:n.toLocaleTimeString(`fr-FR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`}),"time-full":n.toLocaleTimeString(`fr-FR`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`})};for(let e=1;e<=6;e+=1){let n=t?`${`#`.repeat(e)} ${t}`:``;r[`titre-h${e}`]=n,r[`title-h${e}`]=n,r[`titre-${e}`]=n,r[`title-${e}`]=n}for(let e of g){let t=e.token.toLowerCase();t in r||(r[t]=``);for(let n of e.aliases??[]){let e=n.toLowerCase();!(e in r)&&t in r&&(r[e]=r[t])}}return r}function H(e,t={}){let n=Me(t);return String(e??``).replace(/\{\{\s*([a-z0-9_-]+)\s*\}\}/gi,(e,t)=>{let r=String(t??``).toLowerCase();return r in n?n[r]:e})}function Ne(e,t,n={}){let r=e?.folderId??null,i=n.folderId??null,a=String(n.title??``).trim();if(r&&i===r)return``;let o=a.toLowerCase(),s=e?.rules??[];for(let e of s.filter(e=>e.type===`title-exact`))if(e.pattern?.toLowerCase()===o)return H(V(t,e.templateNoteId),{title:a});for(let e of s.filter(e=>e.type===`title-contains`)){let n=e.pattern?.toLowerCase()??``;if(n&&o.includes(n))return H(V(t,e.templateNoteId),{title:a})}for(let e of s.filter(e=>e.type===`folder`))if(e.folderId&&e.folderId===i)return H(V(t,e.templateNoteId),{title:a});for(let e of s.filter(e=>e.type===`default`))return H(V(t,e.templateNoteId),{title:a});return``}function Pe(e){return _.find(t=>t.type===e)?.label??e}var Fe=`settings`,U=`notes_vault_settings`,W=`notes_extensions`,G=`notes_template_prefs`;function Ie(e,t=U){return e?.code===`PGRST204`&&typeof e.message==`string`&&e.message.includes(`'${t}'`)}function K(){return{extensions:R(),templatePrefs:v()}}function q(e){let t={};if(!e||typeof e!=`object`||Array.isArray(e))return t[A]=K(),t;let n=e;for(let[e,r]of Object.entries(n)){if(!r||typeof r!=`object`||Array.isArray(r))continue;let n=r;t[e]={extensions:z(n.extensions),templatePrefs:B(n.templatePrefs)}}return t.root||(t[A]=K()),t}async function J(e,n){await t(n);let{data:r,error:i}=await e.from(Fe).select(`${U}, ${W}, ${G}`).eq(`user_id`,n).maybeSingle();if(i){if(Ie(i))return console.warn(`Colonne ${U} absente. Exécute scripts/migrate-settings-notes-vault-settings.sql dans Supabase.`),{store:q(null),migrated:!1};throw i}if(r?.[U]!=null)return{store:q(r[U]),migrated:!1};let a=q(null);return r?.[W]!=null&&(a[A].extensions=z(r[W])),r?.[G]!=null&&(a[A].templatePrefs=B(r[G])),{store:a,migrated:!0}}async function Y(e,n,r){if(!n)return r;await t(n);let{error:i}=await e.from(Fe).update({[U]:r}).eq(`user_id`,n);if(i)throw Ie(i)?Error(`Colonne ${U} absente. Exécute scripts/migrate-settings-notes-vault-settings.sql dans Supabase.`):i;return r}async function Le(e,t,n=null){if(!t)return R();let{store:r,migrated:i}=await J(e,t),a=I(n);if(r[a]||(r[a]=K()),i)try{await Y(e,t,r)}catch(e){console.warn(`Migration notes_vault_settings impossible:`,e)}return r[a].extensions}async function Re(e,t,n,r){let{store:i}=await J(e,t),a=I(n);return i[a]||(i[a]=K()),i[a].extensions=z(r),await Y(e,t,i),i[a].extensions}async function ze(e,t,n=null){if(!t)return v();let{store:r,migrated:i}=await J(e,t),a=I(n);if(r[a]||(r[a]=K()),i)try{await Y(e,t,r)}catch(e){console.warn(`Migration notes_vault_settings impossible:`,e)}return r[a].templatePrefs}async function Be(e,t,n,r){let{store:i}=await J(e,t),a=I(n);return i[a]||(i[a]=K()),i[a].templatePrefs=B(r),await Y(e,t,i),i[a].templatePrefs}async function Ve(e,t,n){let{store:r}=await J(e,t),i=I(n);return r[i]||(r[i]=K(),await Y(e,t,r)),r[i]}async function He(e,t,n){let{store:r}=await J(e,t),i=I(n);r[i]&&(delete r[i],await Y(e,t,r))}function X(e){let t=String(e?.message??``);return e?.code===`PGRST204`&&(t.includes(`'note_id'`)||t.includes(`'todo_item_id'`)||t.includes(`'status'`))}function Z(e){return`Note - ${String(e??``).trim()||`Sans titre`}`}async function Q(e,t,n=null){if(!t)return!1;try{return Oe(await Le(e,t,n??null),le)}catch(e){return console.error(`note status prefs:`,e),!1}}function Ue(e){return d(String(e?.status_set_at??``).slice(0,10))||d(String(e?.created_at??``).slice(0,10))||r()}async function We(e,t,r,i){try{await k(e,t,r,{todoItemId:i})}catch(e){if(!X(e))throw e}if(i)try{await n(e,t,i,{note_id:r})}catch(e){if(!X(e))throw e}}async function $(e,t,n,r){if(!t||!n?.id)return n;let o=i(r||Ue(n)),s=Z(n.title),c=null;n.todo_item_id&&(c=(await a(e,t)).find(e=>e.id===n.todo_item_id)??null),c||=(await a(e,t)).find(e=>e.note_id===n.id)??null;try{c=c?await p(e,t,c.id,{nom:s,description:c.description||``,frequence:f.WEEK_GOAL,date_echeance:o,heure:c.heure,is_promesse:!!c.is_promesse,quantite_cible:c.quantite_cible,reminder:!!c.reminder,reminder_time:c.reminder_time,note_id:n.id}):await l(e,t,{nom:s,description:``,frequence:f.WEEK_GOAL,date_echeance:o,is_promesse:!1,note_id:n.id})}catch(e){if(X(e))return console.warn(`Colonnes note↔todo absentes. Exécute scripts/migrate-notes-status-todos.sql`),n;throw e}return await We(e,t,n.id,c.id),{...n,todo_item_id:c.id}}async function Ge(e,t,n,s,l={}){if(!t||!n?.id||!l.skipEnabledCheck&&!await Q(e,t,n.vault_id??null))return n;let u=x(n.status),f=x(s),p={status:f||null};f===b.A_TRAITER&&u!==b.A_TRAITER?p.statusSetAt=new Date().toISOString():f===b.A_TRAITER?f===b.A_TRAITER&&n.status_set_at&&(p.statusSetAt=n.status_set_at):f||(p.statusSetAt=null);let m=await k(e,t,n.id,p);if(m={...n,...m},f===b.A_TRAITER)return m=await $(e,t,m),m;let h=null,g=await a(e,t);if(m.todo_item_id&&(h=g.find(e=>e.id===m.todo_item_id)??null),h||=g.find(e=>e.note_id===m.id)??null,f===b.FAIT&&h){let n=d(h.date_echeance)||i(r());return await o(e,t,h,n,!0),m}return!f&&h?(await c(e,t,h.id),m=await k(e,t,m.id,{todoItemId:null}),{...m,todo_item_id:null}):m}async function Ke(t,n){if(!n)return{rolled:0,completed:0};let o=[];try{o=await de(t,n)}catch(e){if(X(e))return{rolled:0,completed:0};throw e}let c=o.filter(e=>x(e.status)===b.A_TRAITER);if(!c.length)return{rolled:0,completed:0};let l=new Map;async function m(e){let r=e||`root`;if(l.has(r))return l.get(r);let i=await Q(t,n,e);return l.set(r,i),i}let h=await a(t,n),g=i(r()),ee=e(await s(t,n,i(d(new Date(Date.now()-1344*60*60*1e3).toISOString().slice(0,10))||g),g)),_=0,v=0;for(let e of c){if(!await m(e.vault_id??null))continue;let r=e.todo_item_id&&h.find(t=>t.id===e.todo_item_id)||h.find(t=>t.note_id===e.id)||null;if(!r){await $(t,n,e,g),_+=1;continue}if(r.frequence!==f.WEEK_GOAL)continue;let i=d(r.date_echeance);if(i){if(i>=g){let a=Z(e.title);r.nom!==a&&await p(t,n,r.id,{nom:a,description:r.description||``,frequence:f.WEEK_GOAL,date_echeance:i,heure:r.heure,is_promesse:!!r.is_promesse,quantite_cible:r.quantite_cible,reminder:!!r.reminder,reminder_time:r.reminder_time,note_id:e.id});continue}if(u(r,i,ee)){await k(t,n,e.id,{status:b.FAIT,statusSetAt:e.status_set_at}),v+=1;continue}await p(t,n,r.id,{nom:Z(e.title??r.nom),description:r.description||``,frequence:f.WEEK_GOAL,date_echeance:g,heure:r.heure,is_promesse:!!r.is_promesse,quantite_cible:r.quantite_cible,reminder:!!r.reminder,reminder_time:r.reminder_time,note_id:e.id}),_+=1}}return{rolled:_,completed:v}}async function qe(e,t,n,r){if(!t||!n?.note_id)return null;try{let i=await O(e,t,n.note_id);return!i||!await Q(e,t,i.vault_id??null)?null:r?await k(e,t,i.id,{status:b.FAIT,statusSetAt:i.status_set_at}):x(i.status)===b.FAIT?await k(e,t,i.id,{status:b.A_TRAITER,statusSetAt:i.status_set_at||new Date().toISOString()}):i}catch(e){if(X(e))return null;throw e}}async function Je(e,t,n){if(!t||!n?.id||!n.todo_item_id||x(n.status)!==b.A_TRAITER)return;let r=(await a(e,t)).find(e=>e.id===n.todo_item_id);if(!r)return;let i=Z(n.title);r.nom!==i&&await p(e,t,r.id,{nom:i,description:r.description||``,frequence:r.frequence||f.WEEK_GOAL,date_echeance:r.date_echeance,heure:r.heure,is_promesse:!!r.is_promesse,quantite_cible:r.quantite_cible,reminder:!!r.reminder,reminder_time:r.reminder_time,note_id:n.id})}export{F as A,le as B,Ce as C,M as D,N as E,O as F,v as G,m as H,de as I,k as L,fe as M,pe as N,xe as O,ge as P,b as R,ve as S,we as T,ee as U,h as V,_ as W,R as _,qe as a,be as b,ze as c,Be as d,Pe as f,L as g,z as h,Je as i,De as j,Ee as k,He as l,Oe as m,Q as n,Ve as o,Ne as p,Ke as r,Le as s,Ge as t,Re as u,_e as v,Te as w,j as x,ye as y,ce as z};