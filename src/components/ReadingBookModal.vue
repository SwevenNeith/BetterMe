...
import { createSpoilChapter, deleteSpoilChapter, listSpoilChapters, updateSpoilChapter } from '../services/readingSpoilChapters.js'
...
async function onAddSpoilChapter(payload) {
  if (!props.userId || !props.book?.id || spoilSaving.value) return

  spoilSaving.value = true
  spoilError.value = ''
  try {
    await createSpoilChapter(supabase, props.userId, props.book.id, payload)
    await loadSpoilChapters()
  } catch (err) {
    console.error(err)
    spoilError.value = err.message || 'Impossible d’ajouter le chapitre.'
  } finally {
  ...
  <Teleport to="body">
    ...
    >
      <ReadingBookFiche
        ...
        @add-spoil-chapter="onAddSpoilChapter"
